# DFRobot_BMI323 API 列表



## 枚举 / 结构体

- `eAccelRange_t`：加速度计量程枚举，支持 ±2g / ±4g / ±8g / ±16g。
- `eGyroRange_t`：陀螺仪量程枚举，支持 ±125dps ~ ±2000dps。
- `eAccelODR_t`：加速度计输出数据率，0.78125 Hz ~ 6400 Hz。
- `eGyroODR_t`：陀螺仪输出数据率，0.78125 Hz ~ 6400 Hz。
- `eInterruptSource_t`：中断引脚选择，`eINT1` 或 `eINT2`。
- `eAxis_t`：轴选择掩码，支持 X / Y / Z / XYZ。
- `sSensorData`：三轴数据结构，成员为 `float x/y/z`。

## 核心初始化

- `DFRobot_BMI323(TwoWire *wire = &Wire)`：构造函数，可显式传入 `TwoWire` 实例。
- `~DFRobot_BMI323()`：析构函数，释放内部指针引用。
- `bool begin(void)`：初始化 I2C 接口、芯片寄存器与特性上下文，成功返回 `true`。

## 传感器配置 / 读数

- `bool configAccel(eAccelODR_t odr, eAccelRange_t range)`：配置加速度计 ODR、量程、带宽与平均次数，并缓存量程用于物理单位转换。
- `bool configGyro(eGyroODR_t odr, eGyroRange_t range)`：配置陀螺仪 ODR、量程、带宽、模式与平均次数，并缓存量程。
- `bool readAccelGyro(sSensorData *accel, sSensorData *gyro)`：一次性读取加速度计与陀螺仪原始数据，转换成 g/dps 返回。
- `int8_t getAccelGyroData(int16_t *data)`：兼容 BMI160 风格的原始六轴读取（前 3 个为陀螺仪，后 3 个为加速度计）。

## 计步功能

- `bool enableStepCounterInterrupt(eInterruptSource_t pin)`：启用计步器与步检测特性，并将中断映射到指定引脚。
- `int8_t readStepCounter(uint16_t *stepVal)`：读取硬件计步器累计步数，返回 `BMI3_OK` 表示成功。

## 中断状态

- `uint16_t getInterruptStatus(void)`：读取 BMI323 中断状态寄存器，返回 Bitmask。
